...
//自动提取标签
function autotag() {
    var title = document.forms["jnr"].ntitle.value
    ...
    document.forms["jnr"].atuotag.disabled = true;
    if (title.length < 1) {
        alert("错误提示：没有填写标题！    ");
        document.forms["jnr"].atuotag.disabled = false;
        return false
    }
    if (Message.length < 1) {
        alert("错误提示：内容不能为空！    ");
			...
			if(xmlHttp.readyState == 4) {
				if(xmlHttp.status == 200) {
					...
					if (TempStr == 0) {
						...
						document.forms["jnr"].nkeyword.value += TempStr
					}
				}
				else{
				alert("Ajax无返回！")
